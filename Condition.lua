...
local function isSpellKnown(spellId)
	local spellName = Ovale:GetSpellInfoOrNil(spellId)
	...
	for tab = 1, GetNumSpellTabs() do
		...
		for i = offset + 1, offset + numSpells do
			local knownSpellName = GetSpellName(i, BOOKTYPE_SPELL)
			if knownSpellName == spellName then
				return true
			end
		end
	end
...
Ovale.conditions=
{
	...
	-- Test if the player has learned a spell in the spellbook.
	-- 1 : spell id
	-- 2 : "yes" or "no"
	SpellKnown = function(condition)
		return testbool(isSpellKnown(condition[1]), condition[2])
	end,
