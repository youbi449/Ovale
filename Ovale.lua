...
local function Ovale_FormatShortcut(key)
	if not key then
		return nil
	end
	local KeyBound = LibStub("LibKeyBound-1.0", true)
	if KeyBound and KeyBound.ToShortKey then
		return KeyBound:ToShortKey(key)
	end
	return GetBindingText(key, "KEY_")
end

local function Ovale_GetFirstBindingKey(binding)
	local key = GetBindingKey(binding)
	return Ovale_FormatShortcut(key)
end

local function Ovale_GetBartenderShortcut(id)
	for i = 1, 120 do
		local button = _G["BT4Button"..i]
		if button then
			local action = button.GetActionID and button:GetActionID() or button.action
			if action == id then
				...
				return Ovale_GetFirstBindingKey("CLICK "..button:GetName()..":LeftButton")
			end
		end
	end
end
...
-- MULTIACTIONBAR4BUTTON1..12 => haut gauche (37..48)
	local bartenderKey = Ovale_GetBartenderShortcut(id)
	if bartenderKey then
		return bartenderKey
	end
	local name;
	...
	local key = Ovale_GetFirstBindingKey(name);
--[[	if (not key) then
...
function Ovale:GetActionInfo(element)
	...
	if (action and not actionTexture) then
		...
		actionShortcut = self:ChercherShortcut(action)
